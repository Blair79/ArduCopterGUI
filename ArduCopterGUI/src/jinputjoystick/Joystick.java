...
public class Joystick {
	...
	public Joystick() {
		...
		if (!foundControllers.isEmpty())
			...
			ArduCopterGUI.addControllerName("No controller found!");
	}
	...
	/**
	 * Starts showing controller data on the joywindow.
	 */
	private void startShowingControllerData() {
		while (true) {
			...
			if (!controller.poll()) {
				ArduCopterGUI.showControllerDisconnected();
				break;
			}
			...
			// we add butons panel to joywindow,
			ArduCopterGUI.setControllerButtons(buttonsPanel);
			// set x and y axes,
			ArduCopterGUI.setXYAxis(xAxisPercentage, yAxisPercentage);
			// add other axes panel to joywindow.
			ArduCopterGUI.addAxisPanel(axesPanel);
